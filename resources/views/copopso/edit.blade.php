@extends('layouts.master')
    @section('main_content')
		...
		<form class="form-vertical" id="co-po-matrix" role="form" action="/co/{{$id}}/updatemap" method="POST">
		{{ csrf_field() }}
				...
				@for($i = 1; $i <= 12; $i++)
					...
					<td>
					  <div class="form-group">
					    <input class="form-control" type="text" name="co{{$co->id}}-po{{$i}}" size="5" pattern="[1-3.\-]"" value="{{$value}}" required>
					  </div>
					</td>			
				@endfor	
				@for($i = 1; $i <= 4; $i++)
					...
					<td>
					  <div class="form-group">
					    <input class="form-control" type="text" name="co{{$co->id}}-pso{{$i}}" size="5" pattern="[1-3.\-]" value="{{$value}}" required>
					  </div>
					</td>			
				@endfor
			</tr>
